...
class FacturationChargeAllocationsSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Même tri-état que la section Versements, et pour la même raison : sur un
    // ensemble encore inconnu, `PermissionGate.allows` refusait, et la table
    // des imputations se taisait devant un caissier qui y a droit. Abonné, pour
    // que l'arrivée de l'ensemble en séance rouvre la table.
          ...
          BlocConsumer<StudentChargesBloc, StudentChargesState>(
            ...
            builder: (context, state) {
              ...
              return AnimatedSwitcher(
                ...
                child: () {
                  // Même règle que la section Versements :
                  // `payment_allocations` n'est peuplée que par le flux
                  // paiements. Sans le droit de le lire, la table est vide
                  // par construction — le dire « aucune imputation » serait
                  // affirmer ce qu'on ne sait pas (ADR-015 §6-C). Sur un
                  // ensemble inconnu, on ne le sait pas non plus : on tente
                  // la lecture, et le serveur reste la frontière.
                  if (holding == PermissionHolding.missing) {
                    return StateCard(
                  ...
                  if (state.allocationsStatus == StudentChargesStatus.failure &&
                    ...
                    return StateCard(
                      ...
                      message: state.allocationsErrorType.localizedMessage(
                        l10n,
                      ),

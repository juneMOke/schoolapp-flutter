...
enum AcademicInfoSection { previousYear, targetYear, all }
...
class AcademicInfoStep extends StatefulWidget {
  ...
  final AcademicInfoSection section;
  ...
  const AcademicInfoStep({
    ...
    this.flowStepIndex,
    this.section = AcademicInfoSection.all,
  });

  @override
  State<AcademicInfoStep> createState() => AcademicInfoStepState();
}

class AcademicInfoStepState extends State<AcademicInfoStep> {
  // Controllers
  ...
  bool get _showPreviousSection =>
      widget.section == AcademicInfoSection.all ||
      widget.section == AcademicInfoSection.previousYear;

  bool get _showTargetSection =>
      widget.section == AcademicInfoSection.all ||
      widget.section == AcademicInfoSection.targetYear;
  ...
  @override
  void didUpdateWidget(covariant AcademicInfoStep oldWidget) {
  ...
  void _recomputeFormState({bool notifyParent = true}) {
    ...
    final targetSectionDirty =
        currYear != _initialCurrYear ||
        _academicYearId != _initialAcademicYearId ||
        _selectedSchoolLevelGroupId != _initialSchoolLevelGroupId ||
        _selectedSchoolLevelId != _initialSchoolLevelId;

    final validNow =
        (!_showPreviousSection || previousSectionValid) &&
        (!_showTargetSection || targetSectionValid);

    final dirtyNow =
        (_showPreviousSection && previousSectionDirty) ||
        (_showTargetSection && targetSectionDirty);
  ...
  List<String> _buildValidationErrors(AppLocalizations l10n) {
    ...
    if (_showTargetSection) {
      if (_currYearController.text.trim().isEmpty || _academicYearId.isEmpty) {
  ...
  void _onSave() {
    ...
    if (widget.section == AcademicInfoSection.targetYear) {
      _studentBloc.add(
  ...
  Widget _buildAcademicContent({required bool isLoading}) {
    final l10n = AppLocalizations.of(context)!;
    ...
    return BlocBuilder<BootstrapBloc, BootstrapState>(
      builder: (context, bootstrapState) {
        ...
        return AcademicInfoStepBody(
          ...
          showPreviousSection: _showPreviousSection,
          showTargetSection: _showTargetSection,
          onSave: _onSave,
          ...
          prevYearError: showValidation && _prevYearController.text.trim().isEmpty
              ...
              : null,
          prevSchoolError: showValidation && _prevSchoolController.text.trim().isEmpty
              ? l10n.requiredFieldError(l10n.schoolLabel)
              : null,
          prevCycleError: showValidation && _prevCycleController.text.trim().isEmpty
              ? l10n.requiredFieldError(l10n.schoolCycle)
              : null,
          prevLevelError: showValidation && _prevLevelController.text.trim().isEmpty
          ...
          levelError: showValidation && _selectedSchoolLevelId.isEmpty
              ...
              : null,
          prevYearChanged: _isChanged(_prevYearController.text, _initialPrevYear),
          prevSchoolChanged: _isChanged(
            _prevSchoolController.text,
            _initialPrevSchool,
          ),
          prevCycleChanged: _isChanged(_prevCycleController.text, _initialPrevCycle),
          prevLevelChanged: _isChanged(_prevLevelController.text, _initialPrevLevel),
          prevRateChanged: _normalizeRate(_prevRateController.text) != _initialPrevRate,
          prevRankChanged: _normalizeRank(_prevRankController.text) != _initialPrevRank,
          validatedPreviousYearChanged:
              _validatedPreviousYear != _initialValidatedPreviousYear,
          groupChanged: _selectedSchoolLevelGroupId != _initialSchoolLevelGroupId,
      ...
      return BlocProvider<StudentBloc>.value(
        ...
        child: BlocConsumer<StudentBloc, StudentState>(
          ...
          builder: (context, state) {
            ...
            return _buildAcademicContent(isLoading: isLoading);
          },
        ),
      );
    }
      ...
      child:
          BlocConsumer<EnrollmentAcademicInfoBloc, EnrollmentAcademicInfoState>(
            ...
            builder: (context, academicState) {
              final isLoading =
                  academicState.status == EnrollmentAcademicInfoStatus.loading;
              return _buildAcademicContent(isLoading: isLoading);
            },
          ),
    );

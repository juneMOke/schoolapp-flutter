...
/// Clé `sync_meta` du curseur de pull des classes.
const String kClassroomsResource = 'classrooms';
...
class _CycleAttempt {
  final Either<Failure, ClassroomPullOutcome> result;
  final bool rejectedCursor;
  const _CycleAttempt(this.result, {this.rejectedCursor = false});
}

/// Pull KEYSET des classes (CF2, re-contracté 2026-07-27) — miroir *lecture*
/// de `SPEC_Frontend_Classroom_Offline_V1` (`GET /sync/classrooms`). Ressource
/// **indépendante** du roster (curseur séparé) : un flux peut avoir plusieurs
/// pages à tirer pendant que l'autre est déjà à jour, sans synchro artificielle.
/// Jeton opaque mémorisé à CHAQUE page (`nextCursor` = progression → reprise
/// après coupure), remplacé par `nextWatermark` en fin de cycle. Calque exact
/// de `ClassroomTransferPullRepositoryImpl`.
class ClassroomPullRepositoryImpl implements ClassroomPullRepository {
  final ClassroomPullApi _api;
  final ClassroomLocalDataSource _localDataSource;
  final SyncMetaDao _syncMetaDao;
  final Map<String, dynamic> _requiredAuth;
  final Clock _now;

  static const String resource = kClassroomsResource;
  static const int pageLimit = 100;

  ClassroomPullRepositoryImpl({
    required ClassroomPullApi api,
  ...
  Future<Either<Failure, ClassroomPullOutcome>>? _tail;
  ...
  Future<Either<Failure, ClassroomPullOutcome>> syncClassrooms({
    ...
    final prev = _tail;
    late final Future<Either<Failure, ClassroomPullOutcome>> scheduled;
    final run = prev == null
  ...
  Future<Either<Failure, ClassroomPullOutcome>> _pull(
  ...
  Future<_CycleAttempt> _attemptCycle(
    ...
    try {
      ...
      if (status == 304) {
        ...
        return _CycleAttempt(
          Right(ClassroomPullOutcome.notModifiedAt(syncedAt, kept)),
        );
      }
      return _CycleAttempt(
      ...
      return const _CycleAttempt(
        Left(ServerFailure('Invalid classrooms pull payload')),
      );
    } catch (_) {
      return const _CycleAttempt(
  ...
  Future<ClassroomPullOutcome> _runCycle(
    int syncedAt,
    String academicYearId, {
    required String? from,
  }) async {
    ...
    while (true) {
      final sent = cursor;
      final response = await _api.pullClassrooms(
        _requiredAuth,
        sent,
        pageLimit,
        academicYearId,
      ...
      await _localDataSource.upsertClassrooms(
    ...
    return upserted == 0
        ? ClassroomPullOutcome.notModifiedAt(syncedAt, cursor)
        : ClassroomPullOutcome(
            upserted: upserted,
            notModified: false,
            syncedAt: syncedAt,
            cursor: cursor,

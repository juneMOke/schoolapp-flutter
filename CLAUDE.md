...
| `expense` | Dépenses de fonctionnement : registre, tableau de bord, **file de validation**, et le **circuit** (une dépense est une demande — déposée, tranchée, puis payée) | plan et état dans `DEPENSES_PLAN.md` (partie I = V1, partie II = v2) ; lecture 100 % locale, écriture 100 % outbox ; un geste = **une** entrée d'outbox, en séquence par dépense (le fil est le registre d'ordre) ; 🔴 **rien ne fusionne dans `main` avant la livraison back C0→C3** — les sept routes de geste n'existent pas encore |
